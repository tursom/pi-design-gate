...
文件证据使用项目内路径，`startLine/endLine` 可省略：默认从第1行到EOF；只给起点则读到末尾，只给终点则从第1行开始。结束行超出EOF会读取到实际末尾，并在审查载荷和工具结果的 `fileEvidence` 中返回 `actualRange`（起止行及总行数）；空文件全文范围为0–0。不存在的起点或倒置范围会报告文件、请求范围与实际行数。

取消201行及整文件1MB门槛，小范围可引用大文件。证据内容总预算为80,000 UTF-8字节，超限指出具体引用和剩余预算，不静默截断。引用校验先撤销旧实施许可，成功才进入模型审查；失败明确提示“尚未调用审查模型”，保留此前待决问题。

文件正文是待分析数据，不是人类批准。审查读取捕获请求、已回答问题与实际证据，不仅相信方案中的批准自述。
